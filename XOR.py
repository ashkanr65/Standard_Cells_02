...
class XOR(pya.PCellDeclarationHelper):
  """
  ...
  def impl(self):
    ...
    self.transistor(1, x0, y, w_d, self.n_d, self.l_d, True, False, 0, 0, 0, 3)
    #gates name
    ...
    x1 = x0 + 5*d_x_101
    self.transistor(2, x1, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 3)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x2 = x1 + 5*l_x
    self.transistor(3, x2, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O0", 0.001, 5).move((x2 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #NOR2_1
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)3
    x3 = x2 + 5*d_x_101
    self.transistor(1, x3, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O0", 0.001, 5).move((x3- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x5 = x4 + 5*l_x
    self.transistor(3, x5, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O1", 0.001, 5).move((x5 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #NOR2_2
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)6
    x6 = x5 + 5*d_x_101
    self.transistor(1, x6, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O0", 0.001, 5).move((x6- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)7
    x7 = x6 + 5*d_x_101
    self.transistor(2, x7, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x8 = x7 + 5*l_x
    self.transistor(3, x8, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O2", 0.001, 5).move((x8 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #NOR2_3
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)9
    x9 = x8 + 5*d_x_101
    self.transistor(1, x9, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O2", 0.001, 5).move((x9- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O1", 0.001, 5).move((x10- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O3", 0.001, 5).move((x11 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #clock inverter
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)12
    x12 = x11 + 5*d_x_101
    self.transistor(1, x12, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("O3", 0.001, 5).move((x12- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("out", 0.001, 5).move((x13 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    interconnect = pya.Path([pya.Point((x1-gate_edge + (via + ov)/2)/dbu,gate_connection),
        pya.Point((x1-gate_edge + (via + ov)/2)/dbu,gate_connection - path_step),
    ...
    # X5 to X10 connection
    interconnect = pya.Path([pya.Point((x5-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        pya.Point((x5-gate_edge + (via + ov)/2)/dbu,-gate_connection + path_step),
    ...
    interconnect = pya.Path([pya.Point((x11-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        pya.Point((x12-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)
    ...
    Out = pya.Path([pya.Point((x13-gate_edge + (via + ov)/2)/dbu, gate_connection ),
        pya.Point((x13-gate_edge + (via + ov)/2)/dbu, gate_connection + 3*path_step),
        ],path_width_dbu)
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    self.layout().register_pcell("XOR", XOR())
