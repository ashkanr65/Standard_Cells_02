...
class DLatch(pya.PCellDeclarationHelper):
  """
  ...
  def impl(self):
    ...
    Out = pya.Path([pya.Point((x8-gate_edge + (via + ov)/2)/dbu, gate_connection ),
        pya.Point((x8-gate_edge + (via + ov)/2)/dbu, gate_connection + 3*path_step),
        ],path_width_dbu)
    self.cell.shapes(txt).insert(Out)

    # Q' Input
    Out = pya.Path([pya.Point((x9-gate_edge + (via + ov)/2)/dbu, gate_connection ),
        pya.Point((x9-gate_edge + (via + ov)/2)/dbu, gate_connection + 3*path_step),
        ],path_width_dbu)
    self.cell.shapes(txt).insert(Out)
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    # Set the description
    self.description = "DLatch"
    
    # Create the PCell declarations
    self.layout().register_pcell("DLatch", DLatch())
    # That would be the place to put in more PCells ...
    
    # Register us with the name "MyLib".
    # If a library with that name already existed, it will be replaced then.
    self.register("DLatch")
