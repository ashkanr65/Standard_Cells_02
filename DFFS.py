...
class DFFS_V2(pya.PCellDeclarationHelper):
  ...
  def display_text_impl(self):
    ...
    return "STD Cell DFFS:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!SET", 0.001, 5).move((x20 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    # X14 to X22 connection
    interconnect = pya.Path([pya.Point((x14-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        pya.Point((x14-gate_edge + (via + ov)/2)/dbu,-gate_connection + path_step),
    ...
    interconnect = pya.Path([pya.Point((x17-gate_edge + (via + ov)/2)/dbu,gate_connection),
    ...
    # SET Input
    Input = pya.Path([pya.Point((x20-gate_edge + (via + ov)/2)/dbu, -gate_connection ),
        pya.Point((x20-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(Input)
    ...
    if(self.pad):
        ...
        vin2 = pya.Path([
            ...
            pya.Point((x20-gate_edge + (via + ov)/2)/dbu,-2*path_step + gate_in),
            pya.Point((x20-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
        ],path_width_dbu)
