...
class DFFR_V2(pya.PCellDeclarationHelper):
  """
  ...
  def impl(self):
    ...
    x14 = x13 + 5*d_x_0
    self.transistor(1, x14, y, w_d, self.n_d, self.l_d, True, False, 100, d_x_0, d_x_0, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!Reset", 0.001, 5).move((x14 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)15
    x15 = x14 + 5*d_x_101
    self.transistor(2, x15, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x16 = x15 + 5*l_x
    self.transistor(3, x16, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("Q'", 0.001, 5).move((x16 - gate_edge) / dbu, Top_Path + 20 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #D inverter
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)17
    x17 = x16 + 5*d_x_101
    self.transistor(1, x17, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x18 = x17 + 5*l_x
    self.transistor(3, x18, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x19 = x18 + 5*d_x_101
    self.transistor(0, x19, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("out1'", 0.001, 5).move((x19 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)20
    x20 = x19 + 5*d_x_0
    self.transistor(1, x20, y, w_d, self.n_d, self.l_d, True, False, 100, d_x_0, d_x_0, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("clk'", 0.001, 5).move((x20 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)21
    x21 = x20 + 5*d_x_101
    self.transistor(2, x21, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x22 = x21 + 5*l_x
    self.transistor(3, x22, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 3)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    interconnect = pya.Path([pya.Point((x12-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        ...
        pya.Point((x20-gate_edge + (via + ov)/2)/dbu,-gate_connection + path_step),
        pya.Point((x20-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)

    # X13 to X17 connection
    ...
    interconnect = pya.Path([pya.Point((x15-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        ...
        pya.Point((x22-gate_edge + (via + ov)/2)/dbu,-gate_connection - path_step),
        pya.Point((x22-gate_edge + (via + ov)/2)/dbu,-gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)
    
    # X16 to X21 connection
    interconnect = pya.Path([pya.Point((x16-gate_edge + (via + ov)/2)/dbu,gate_connection),
        pya.Point((x16-gate_edge + (via + ov)/2)/dbu,gate_connection + path_step),
        pya.Point((x21-gate_edge + (via + ov)/2)/dbu,gate_connection + path_step),
        pya.Point((x21-gate_edge + (via + ov)/2)/dbu,gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)

    # X18 to X19 connection
    interconnect = pya.Path([pya.Point((x18-gate_edge + (via + ov)/2)/dbu,gate_connection),
