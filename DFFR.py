...
class DFFR_V2(pya.PCellDeclarationHelper):
  """
  The PCell declaration for the DFFR.
  """
  ...
  def impl(self):
    ...
    interconnect = pya.Path([pya.Point((x15-gate_edge + (via + ov)/2)/dbu,y/dbu-gate_connection),
        ...
        pya.Point((x22-gate_edge + (via + ov)/2)/dbu,y/dbu-gate_connection - path_step),
        pya.Point((x22-gate_edge + (via + ov)/2)/dbu,y/dbu-gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)
    
    # X16 to X21 connection
    interconnect = pya.Path([pya.Point((x16-gate_edge + (via + ov)/2)/dbu,y/dbu+gate_connection),
    ...
    interconnect = pya.Path([pya.Point((x18-gate_edge + (via + ov)/2)/dbu,y/dbu+gate_connection),
    ...
    # clk Input
    Input = pya.Path([pya.Point((x5-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection + path_step ),
        pya.Point((x5-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(Input)

    # RST Input
    Input = pya.Path([pya.Point((x14-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection ),
    ...
    if(self.pad):
        ...
        name = pya.TextGenerator.default_generator().text\
            ("DFFR", 0.001, 35).move(-300/dbu, -150 / dbu)
