...
class Ring_NOR3(pya.PCellDeclarationHelper):
  ...
  def display_text_impl(self):
    ...
    return "STD Cell Ring_Osc_NOR3:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    x2 = x1 + 5*d_x_101
    self.transistor(2, x2, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x4 = x3 + 5*d_x_101
    self.transistor(1, x4, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0", 0.001, 5).move((x4- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)5
    x5 = x4 + 5*d_x_101
    self.transistor(2, x5, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x5- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)6
    x6 = x5 + 5*d_x_101
    self.transistor(2, x6, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x6- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x8 = x7 + 5*d_x_101
    self.transistor(1, x8, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0", 0.001, 5).move((x8- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)9
    x9 = x8 + 5*d_x_101
    self.transistor(2, x9, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x9- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)10
    x10 = x9 + 5*d_x_101
    self.transistor(2, x10, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x10- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x12 = x11 + 5*d_x_101
    self.transistor(1, x12, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0", 0.001, 5).move((x12- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)13
    x13 = x12 + 5*d_x_101
    self.transistor(2, x13, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x13- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)14
    x14 = x13 + 5*d_x_101
    self.transistor(2, x14, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x14- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x16 = x15 + 5*d_x_101
    self.transistor(1, x16, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0", 0.001, 5).move((x16- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)17
    x17 = x16 + 5*d_x_101
    self.transistor(2, x17, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x17- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)18
    x18 = x17 + 5*d_x_101
    self.transistor(2, x18, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x18- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    if (self.buffer == True):
        ...
        self.transistor(1, x20, y, w_d, rt*self.n_d, self.l_d, True, False, 0, l_x, 0, 1)
        #gates name
        # iTregion = pya.TextGenerator.default_generator().text\
        ...
        self.transistor(2, x21, y, w_d, rt*self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
        #gates name
        # iTregion = pya.TextGenerator.default_generator().text\
        ...
        self.transistor(2, x22, y, w_d, rt*self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
        #gates name
        # iTregion = pya.TextGenerator.default_generator().text\
    ...
    # Out
    if (self.buffer == True):
        Out = pya.Path([pya.Point((x23-gate_edge_buffer + (via + ov)/2)/dbu, y/dbu+gate_connection ),
            pya.Point((x23-gate_edge_buffer + (via + ov)/2)/dbu, Top_Edge),
            ],path_width_dbu)
        self.cell.shapes(gc).insert(Out)

    else:
        Out = pya.Path([pya.Point((x19-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection ),
    ...
    interconnect = pya.Path([pya.Point((xp-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection),
        pya.Point((x1-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection),
        ],path_width_dbu)
    ...
    if (self.buffer == True):
        interconnect = pya.Path([pya.Point((x19-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
            pya.Point((x22-gate_edge_buffer + (via + ov)/2)/dbu, y/dbu+gate_connection),
            ],path_width_dbu)
        self.cell.shapes(gc).insert(interconnect)

    # IN_OUT
    interconnect = pya.Path([pya.Point((x19-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
        pya.Point((x19-gate_edge + (via + ov)/2)/dbu, y/dbu),
        pya.Point((x2-gate_edge + (via + ov)/2)/dbu, y/dbu),
    ...
    if(self.pad):
        ...
        name = pya.TextGenerator.default_generator().text\
            ("Ring_NOR3", 0.001, 35).move(-450/dbu, -150 / dbu)
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    # Set the description
    self.description = "Ring_NOR3"
    
    # Create the PCell declarations
    self.layout().register_pcell("Ring_NOR3", Ring_NOR3())
    # That would be the place to put in more PCells ...
    
    # Register us with the name "MyLib".
    # If a library with that name already existed, it will be replaced then.
    self.register("Ring_NOR3")
