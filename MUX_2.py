...
class MUX_2_V2(pya.PCellDeclarationHelper):
  """
  The PCell declaration for the MUX.
  """
  ...
  def display_text_impl(self):
    ...
    return "STD Cell MUX2:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S0", 0.001, 5).move((x1- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)2
    x2 = x1 + 5*l_x
    self.transistor(3, x2, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, l_x, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0_O", 0.001, 5).move((x2- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Not S0
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)3
    x3 = x2 + 5*d_x_101
    self.transistor(1, x3, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, 2*d_x_101, 3)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S0", 0.001, 5).move((x3- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)4
    x4 = x3 + 5*l_x
    self.transistor(3, x4, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, l_x, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S'0", 0.001, 5).move((x4- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Input I1 and S'0
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)5
    x5 = x4 + 5*d_x_101
    self.transistor(1, x5, y, w_d, self.n_d, self.l_d, True, False, 0, 2*d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S'0", 0.001, 5).move((x5- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1_O", 0.001, 5).move((x7 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Input I0_out and I1_out
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)8
    x8 = x7 + 5*d_x_101
    self.transistor(1, x8, y, w_d, self.n_d, self.l_d, True, False, 0, 2*d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1_o", 0.001, 5).move((x8- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)9
    x9 = x8 + 5*d_x_101
    self.transistor(2, x9, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0_o", 0.001, 5).move((x9- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)10
    x10 = x9 + 5*l_x
    self.transistor(3, x10, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("out", 0.001, 5).move((x10 - gate_edge) / dbu, Top_Path + 20 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    if(self.pad):
        ...
        vin0 = pya.Path([
            ...
            pya.Point((x6-gate_edge + (via + ov)/2)/dbu,-2*path_step + gate_in),
            pya.Point((x6-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
        ],path_width_dbu)
        self.cell.shapes(gc).insert(vin0)
        ...
        if(self.sel == False):
            ...
            sel = pya.Path([
                pya.Point((x3-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
                pya.Point((x3-gate_edge + (via + ov)/2)/dbu, Bottom_rail - self.rail*finger_width_dbu/2),
            ],path_width_dbu)
            ...
            sel = pya.Path([
                pya.Point((x3-gate_edge + (via + ov)/2)/dbu, Top_Edge),
                pya.Point((x3-gate_edge + (via + ov)/2)/dbu, Top_rail + self.rail*finger_width_dbu/2),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(sel)
            # via connection
        ...
        vout = pya.Path([
            ...
            pya.Point((x10-gate_edge + (via + ov)/2)/dbu, gate_out),
            pya.Point((x10-gate_edge + (via + ov)/2)/dbu, Top_Edge),
        ],path_width_dbu)
