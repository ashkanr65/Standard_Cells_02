...
class MUX_2(pya.PCellDeclarationHelper):
  ...
  def impl(self):
    ...
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x0- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1_O", 0.001, 5).move((x2- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Not S0
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)3
    x3 = x2 + 5*d_x_101
    self.transistor(1, x3, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, 2*d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0_O", 0.001, 5).move((x7 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0_out", 0.001, 5).move((x8- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1_out", 0.001, 5).move((x9- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    Input = pya.Path([pya.Point((x6-gate_edge + (via + ov)/2)/dbu, -gate_connection ),
        pya.Point((x6-gate_edge + (via + ov)/2)/dbu, -gate_connection - 3*path_step),
        ],path_width_dbu)
    self.cell.shapes(txt).insert(Input)
    ...
    Out = pya.Path([pya.Point((x10-gate_edge + (via + ov)/2)/dbu, gate_connection ),
        pya.Point((x10-gate_edge + (via + ov)/2)/dbu, gate_connection + 3*path_step),
        ],path_width_dbu)
