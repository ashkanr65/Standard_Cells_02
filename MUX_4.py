...
class MUX_4(pya.PCellDeclarationHelper):
  """
  ...
  def display_text_impl(self):
    ...
    return "STD Cell MUX4:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    x1 = xp + 5*d_x_101
    self.transistor(2, x1, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 3)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S1", 0.001, 5).move((x1- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)2
    x2 = x1 + 5*d_x_101
    self.transistor(2, x2, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 3)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S0", 0.001, 5).move((x2- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)3
    x3 = x2 + 5*l_x
    self.transistor(3, x3, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, l_x, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0_O", 0.001, 5).move((x3- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Input I2 and S'0 and S1
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)4
    x4 = x3 + 5*d_x_101
    self.transistor(1, x4, y, w_d, self.n_d, self.l_d, True, False, 0, 2*d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x4- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S1", 0.001, 5).move((x5- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)6
    x6 = x5 + 5*d_x_101
    self.transistor(2, x6, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!S0", 0.001, 5).move((x6- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)7
    x7 = x6 + 5*l_x
    self.transistor(3, x7, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2_O", 0.001, 5).move((x7 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Not S0
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)8
    x8 = x7 + 5*d_x_101
    self.transistor(1, x8, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, 2*d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S0", 0.001, 5).move((x8- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)9
    x9 = x8 + 5*l_x
    self.transistor(3, x9, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, l_x, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!S0", 0.001, 5).move((x9- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Input I1 and S0 and S'1
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)10
    x10 = x9 + 5*d_x_101
    self.transistor(1, x10, y, w_d, self.n_d, self.l_d, True, False, 0, 2*d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S0", 0.001, 5).move((x10- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)11
    x11 = x10 + 5*d_x_101
    self.transistor(2, x11, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x11- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)12
    x12 = x11 + 5*d_x_101
    self.transistor(2, x12, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!S1", 0.001, 5).move((x12- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)13
    x13 = x12 + 5*l_x
    self.transistor(3, x13, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1_O", 0.001, 5).move((x13 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Not S1
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)14
    x14 = x13 + 5*d_x_101
    self.transistor(1, x14, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, 2*d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("S1", 0.001, 5).move((x14- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)15
    x15 = x14 + 5*l_x
    self.transistor(3, x15, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, l_x, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!S1", 0.001, 5).move((x15- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Input I3 and S'0 and S'1
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)16
    x16 = x15 + 5*d_x_101
    self.transistor(1, x16, y, w_d, self.n_d, self.l_d, True, False, 0, 2*d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!S1", 0.001, 5).move((x16- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)17
    x17 = x16 + 5*d_x_101
    self.transistor(2, x17, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, 2*d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!S0", 0.001, 5).move((x17- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I3", 0.001, 5).move((x18- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I3_O", 0.001, 5).move((x19 - gate_edge) / dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    #Out
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)20
    x20 = x19 + 5*d_x_101
    self.transistor(1, x20, y, w_d, self.n_d, self.l_d, True, False, 0, d_x_101, 2*d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("!out", 0.001, 5).move((x20- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)21
    x21 = x20 + 5*l_x
    self.transistor(3, x21, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("Out", 0.001, 5).move((x21- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    if(self.pad):
        ...
        if (self.sel0 == False):
            ...
            sel = pya.Path([
                pya.Point((x2-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
                pya.Point((x2-gate_edge + (via + ov)/2)/dbu, Bottom_rail - self.rail*finger_width_dbu/2),
            ],path_width_dbu)
            ...
            sel = pya.Path([
                pya.Point((x2-gate_edge + (via + ov)/2)/dbu, Top_Edge),
                pya.Point((x2-gate_edge + (via + ov)/2)/dbu, Top_rail + self.rail*finger_width_dbu/2),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(sel)
            # via connection
            z_via = pya.Path([
                pya.Point((x2-gate_edge + (2*ov)/2)/dbu, Top_Edge + ov_dbu),
                pya.Point((x2-gate_edge + (2*ov)/2)/dbu, Top_rail - ov_dbu + self.rail*finger_width_dbu/2),
            ],via/dbu)
            self.cell.shapes(pv).insert(z_via)
        ...
        if (self.sel1 == False):
            ...
            sel = pya.Path([
                pya.Point((x1-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
                pya.Point((x1-gate_edge + (via + ov)/2)/dbu, Bottom_rail - self.rail*finger_width_dbu/2),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(sel)
            # via connection
            z_via = pya.Path([
            ...
            sel = pya.Path([
                pya.Point((x1-gate_edge + (via + ov)/2)/dbu, Top_Edge),
                pya.Point((x1-gate_edge + (via + ov)/2)/dbu, Top_rail + self.rail*finger_width_dbu/2),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(sel)
            # via connection
            z_via = pya.Path([
                pya.Point((x1-gate_edge + (2*ov)/2)/dbu, Top_Edge + ov_dbu),
                pya.Point((x1-gate_edge + (2*ov)/2)/dbu, Top_rail - ov_dbu + self.rail*finger_width_dbu/2),
            ],via/dbu)
            self.cell.shapes(pv).insert(z_via)
        
        if (self.sel0 == False and self.sel1 == False):
            vin1 = pya.Path([
                pya.Point(-320/dbu, -200/dbu),
        ...
        if (self.sel0 == False and self.sel1 == True):
            vin1 = pya.Path([
                pya.Point(-320/dbu, -200/dbu),
                pya.Point(-320/dbu, gate_in),
                pya.Point((x11-gate_edge + (via + ov)/2)/dbu, gate_in),
                pya.Point((x11-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(vin1)
        if (self.sel0 == True and self.sel1 == False):
            vin1 = pya.Path([
                pya.Point(-320/dbu, -200/dbu),
                pya.Point(-320/dbu, gate_in),
                pya.Point((x4-gate_edge + (via + ov)/2)/dbu, gate_in),
                pya.Point((x4-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(vin1)
        if (self.sel0 == True and self.sel1 == True):
            vin1 = pya.Path([
                pya.Point(-320/dbu, -200/dbu),
                pya.Point(-320/dbu, gate_in),
                pya.Point((x18-gate_edge + (via + ov)/2)/dbu, gate_in),
                pya.Point((x18-gate_edge + (via + ov)/2)/dbu, Bottom_Edge),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(vin1)
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    # Set the description
    self.description = "MUX_4"
    
    # Create the PCell declarations
    self.layout().register_pcell("MUX_4", MUX_4())
