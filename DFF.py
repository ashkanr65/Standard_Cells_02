...
class DFF_V2(pya.PCellDeclarationHelper):
  """
  ...
  def display_text_impl(self):
    ...
    return "STD Cell DFF:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    x21 = x20 + 5*l_x
    self.transistor(3, x21, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 3)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    # Q out
    Out = pya.Path([pya.Point((x20-gate_edge + (via + ov)/2)/dbu, gate_connection ),
    ...
    if(self.pad):
        ...
        if (self.q == True):
            ...
            vout = pya.Path([
                pya.Point(320/dbu, 200/dbu),
                pya.Point(320/dbu, gate_out),
                pya.Point((x20-gate_edge + (via + ov)/2)/dbu, gate_out),
                pya.Point((x20-gate_edge + (via + ov)/2)/dbu, Top_Edge),
            ],path_width_dbu)
            self.cell.shapes(gc).insert(vout)
