...
class Ring_Inv(pya.PCellDeclarationHelper):
  ...
  def display_text_impl(self):
    ...
    return "STD Cell Ring_Osc_Inv:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("Out", 0.001, 5).move((x1 - gate_edge) / dbu, Top_Path + 20 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # Ring_2
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)2
    x2 = x1 + 5*d_x_101
    self.transistor(1, x2, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0", 0.001, 5).move((x2- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)3
    x3 = x2 + 5*l_x
    self.transistor(3, x3, y, w_d, p * self.n_d, self.l_l, True, True, 101, l_x, 0, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("Out", 0.001, 5).move((x1 - gate_edge) / dbu, Top_Path + 20 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x4 = x3 + 5*d_x_101
    self.transistor(1, x4, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x6 = x5 + 5*d_x_101
    self.transistor(1, x6, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x8 = x7 + 5*d_x_101
    self.transistor(1, x8, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    # Out
    if (self.buffer == True):
        Out = pya.Path([pya.Point((x11-gate_edge_buffer + (via + ov)/2)/dbu, y/dbu+gate_connection ),
            pya.Point((x11-gate_edge_buffer + (via + ov)/2)/dbu, Top_Edge),
            ],path_width_dbu)
        self.cell.shapes(gc).insert(Out)

    else:
        Out = pya.Path([pya.Point((x9-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection ),
    ...
    if (self.buffer == True):
        interconnect = pya.Path([pya.Point((x9-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
            pya.Point((x10-gate_edge_buffer + (via + ov)/2)/dbu, y/dbu+gate_connection),
            ],path_width_dbu)
        self.cell.shapes(gc).insert(interconnect)

    # IN_OUT
    interconnect = pya.Path([pya.Point((x9-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
        pya.Point((x9-gate_edge + (via + ov)/2)/dbu, y/dbu),
        pya.Point((xp-gate_edge + (via + ov)/2)/dbu, y/dbu),
        pya.Point((xp-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)
    ...
    if(self.pad):
        ...
        name = pya.TextGenerator.default_generator().text\
            ("Ring_Inverter", 0.001, 35).move(-450/dbu, -150 / dbu)
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    # Set the description
    self.description = "Ring_Inv"
    
    # Create the PCell declarations
    self.layout().register_pcell("Ring_Inv", Ring_Inv())
    # That would be the place to put in more PCells ...
    
    # Register us with the name "MyLib".
    # If a library with that name already existed, it will be replaced then.
    self.register("Ring_Inv")
