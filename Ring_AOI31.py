...
class Ring_AOI31(pya.PCellDeclarationHelper):
  """
  The PCell declaration for the Ring_AOI31.
  """

  def __init__(self):

    # Important: initialize the super class
    super(Ring_AOI31, self).__init__()
  ...
  def display_text_impl(self):
    ...
    return "STD Cell Ring_Osc_AOI31:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    x7 = x6 + 5*d_x_100
    self.transistor(1, x7, y, w_d, self.n_d, self.l_d, True, False, 100, d_x_100, 2*d_x_100, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NA2", 0.001, 5).move((x7- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NO", 0.001, 5).move((x8- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x12 = x11 + 5*d_x_100
    self.transistor(1, x12, y, w_d, self.n_d, self.l_d, True, False, 100, d_x_100, 2*d_x_100, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NA2", 0.001, 5).move((x12- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NO", 0.001, 5).move((x13- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x17 = x16 + 5*d_x_100
    self.transistor(1, x17, y, w_d, self.n_d, self.l_d, True, False, 100, d_x_100, 2*d_x_100, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NA2", 0.001, 5).move((x17- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NO", 0.001, 5).move((x18- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x22 = x21 + 5*d_x_100
    self.transistor(1, x22, y, w_d, self.n_d, self.l_d, True, False, 100, d_x_100, 2*d_x_100, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NA2", 0.001, 5).move((x22- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I_NO", 0.001, 5).move((x23- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    if(self.pad):
        ...
        name = pya.TextGenerator.default_generator().text\
            ("Ring_AOI31", 0.001, 35).move(-450/dbu, -150 / dbu)
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    # Set the description
    self.description = "Ring_AOI31"
    
    # Create the PCell declarations
    self.layout().register_pcell("Ring_AOI31", Ring_AOI31())
    # That would be the place to put in more PCells ...
    
    # Register us with the name "MyLib".
    # If a library with that name already existed, it will be replaced then.
    self.register("Ring_AOI31")
