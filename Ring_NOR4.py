...
class Ring_NOR4(pya.PCellDeclarationHelper):
  """
  The PCell declaration for the Ring_NOR4.
  """

  def __init__(self):

    # Important: initialize the super class
    super(Ring_NOR4, self).__init__()
  ...
  def display_text_impl(self):
    ...
    return "STD Cell Ring_Osc_NOR4:(ratio=" + str((self.r)) + ",Width="+ "90*"+str((self.n_d)) + ")"
  ...
  def impl(self):
    ...
    x1 = xp + 5*d_x_101
    self.transistor(2, x1, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x1- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)2
    x2 = x1 + 5*d_x_101
    self.transistor(2, x2, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x2- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)3
    x3 = x2 + 5*d_x_101
    self.transistor(2, x3, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I3", 0.001, 5).move((x3- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x5=x4 + 5*d_x_101
    self.transistor(1, x5, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I0", 0.001, 5).move((x5- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)6
    x6 = x5 + 5*d_x_101
    self.transistor(2, x6, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I1", 0.001, 5).move((x6- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    
    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)7
    x7 = x6 + 5*d_x_101
    self.transistor(2, x7, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x7- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I3", 0.001, 5).move((x8- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x10=x9 + 5*d_x_101
    self.transistor(1, x10, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x11 = x10 + 5*d_x_101
    self.transistor(2, x11, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x12 = x11 + 5*d_x_101
    self.transistor(2, x12, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x12- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)13
    x13 = x12 + 5*d_x_101
    self.transistor(2, x13, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I3", 0.001, 5).move((x13- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    x15=x14 + 5*d_x_101
    self.transistor(1, x15, y, w_d, self.n_d, self.l_d, True, False, 0, l_x, 0, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x16 = x15 + 5*d_x_101
    self.transistor(2, x16, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x17 = x16 + 5*d_x_101
    self.transistor(2, x17, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I2", 0.001, 5).move((x17- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)

    # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)18
    x18 = x17 + 5*d_x_101
    self.transistor(2, x18, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x21 = x20 + 5*d_x_101
    self.transistor(2, x21, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x22 = x21 + 5*d_x_101
    self.transistor(2, x22, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, d_x_101, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
    ...
    x23 = x22 + 5*d_x_101
    self.transistor(2, x23, y, w_d, self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
    #gates name
    iTregion = pya.TextGenerator.default_generator().text\
        ("I3", 0.001, 5).move((x23- gate_edge)/ dbu, -Top_Path - 25 / dbu)
    self.cell.shapes(txt).insert(iTregion)
    ...
    if (self.buffer == True):
        ...
        x25= x24 + 5*d_x_buffer_0
        self.transistor(1, x25, y, w_d, rt*self.n_d, self.l_d, True, False, 0, l_x, right_ov_load, 1)
        #gates name
        # iTregion = pya.TextGenerator.default_generator().text\
        ...
        x26 = x25 + 5*d_x_101_buffer
        self.transistor(2, x26, y, w_d, rt*self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
        #gates name
        # iTregion = pya.TextGenerator.default_generator().text\
        #     ("I1", 0.001, 5).move((x26- gate_edge)/ dbu, -Top_Path - 25 / dbu)
        # self.cell.shapes(txt).insert(iTregion)

        # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)27
        x27 = x26 + 5*d_x_101_buffer
        self.transistor(2, x27, y, w_d, rt*self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 1)
        #gates name
        # iTregion = pya.TextGenerator.default_generator().text\
        #     ("I1", 0.001, 5).move((x27- gate_edge)/ dbu, -Top_Path - 25 / dbu)
        # self.cell.shapes(txt).insert(iTregion)

        # (level, x, y, w_i, n_i, l_i, bg, Load, In_Con, overlap_left, overlap_rigth, out)28
        x28 = x27 + 5*d_x_101_buffer
        self.transistor(2, x28, y, w_d, rt*self.n_d, self.l_d, True, False, 101, d_x_101, right_ov_load, 0)
        #gates name
        iTregion = pya.TextGenerator.default_generator().text\
    ...
    # Out
    if (self.buffer == True):
        Out = pya.Path([pya.Point((x29-gate_edge_buffer + (via + ov)/2)/dbu, y/dbu+gate_connection ),
            pya.Point((x29-gate_edge_buffer + (via + ov)/2)/dbu, Top_Edge),
            ],path_width_dbu)
        self.cell.shapes(gc).insert(Out)

    else:
        Out = pya.Path([pya.Point((x24-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection ),
    ...
    interconnect = pya.Path([pya.Point((xp-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection),
        pya.Point((x2-gate_edge + (via + ov)/2)/dbu, y/dbu-gate_connection),
        ],path_width_dbu)
    self.cell.shapes(gc).insert(interconnect)
    
    # Out4_In5,6,7,8
    interconnect = pya.Path([pya.Point((x4-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
    ...
    if (self.buffer == True):
        interconnect = pya.Path([pya.Point((x24-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
            pya.Point((x28-gate_edge_buffer + (via + ov)/2)/dbu, y/dbu+gate_connection),
            ],path_width_dbu)
        self.cell.shapes(gc).insert(interconnect)
    
    # IN_OUT
    interconnect = pya.Path([pya.Point((x24-gate_edge + (via + ov)/2)/dbu, y/dbu+gate_connection),
        pya.Point((x24-gate_edge + (via + ov)/2)/dbu, y/dbu),
        pya.Point((x3-gate_edge + (via + ov)/2)/dbu, y/dbu),
...
class MyLib(pya.Library):
  ...
  def __init__(self):
    ...
    # Set the description
    self.description = "Ring_NOR4"
    
    # Create the PCell declarations
    self.layout().register_pcell("Ring_NOR4", Ring_NOR4())
    # That would be the place to put in more PCells ...
    
    # Register us with the name "MyLib".
    # If a library with that name already existed, it will be replaced then.
    self.register("Ring_NOR4")
